...
def send_to_openai(md_content):
    ...
    data = {
        ...
        'prompt': md_content + "\nThe above content is rough draft of some analysis done on a website. Kindly prettify the analysis well and should be in the formatgiven below.  :" + \
...
def write_to_file(data, domain):
    ...
    if not os.path.exists(os.path.join(PATH, 'analysis.md')):
        with open(os.path.join(PATH, 'analysis.md'), "w", encoding="utf-8") as f:
            open(os.path.join(PATH, 'analysis.md'), "w").close()

    # Append the analysis report of the domain to the file
    with open(os.path.join(PATH, 'analysis.md'), "a", encoding="utf-8") as f:
        f.write(f"# Analysis of {domain}\n\n")
        f.write(str(data))

# Function to read the existing.txt file and grab the domain names
...
def process_domain(domain):
    ...
    for chunk in md_chunks:
        ...
        if len(md_chunks) > 5:
            rand_indices = random.sample(range(1, len(md_chunks)-1), 2)
